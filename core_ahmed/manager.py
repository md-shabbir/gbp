...
from gbpservice.nfp.core_ahmed import executor as nfp_executor
from gbpservice.nfp.core_ahmed import log as nfp_logging
from gbpservice.nfp.core_ahmed import sequencer as nfp_sequencer
from gbpservice.nfp.utils.forked_pdb import ForkedPdb

LOG = nfp_logging.getLogger(__name__)
LOG.error = LOG.debug
NfpEventManager = nfp_event.NfpEventManager
EventGraphExecutor = nfp_executor.EventGraphExecutor
...
class NfpResourceManager(NfpProcessManager, NfpEventManager):
    ...
    def __init__(self, conf, controller):
        ...
        # Graph executor
        self.graph_executor = EventGraphExecutor(self)
        # list of poll events to be stopped
        self.poll_event_stop = []
    ...
    def _graph_event(self, event):
        ...
        # Just maintain id here
        event.desc.graph = None
        self._event_cache[event.desc.uuid] = event

        if event.desc.uuid == root:
            # graph = {'id': <>, 'tree': {}, 'root': <>}
            self.graph_executor.execute(graph)


    def _graph_event_complete(self, event):
        #self._scheduled_event_complete(event)
        self.graph_executor.conntinue(event.desc.uuid, graph=event.desc.graph)
    ...
    def _scheduled_new_event(self, event):
        ...
        if not event.sequence:
            # Check if this event is part of any graph
            # and also if it can be dispatched or not
            dispatch = self.graph_executor.dispatch(event.desc.uuid)
            if dispatch:
    ...
    def _stop_poll_event(self, event):
        ...
        poll_event.desc.poll_desc.stopped = True
    ...
    def _non_schedule_event(self, event):
        if event.desc.type == nfp_event.POLL_EVENT:
            ...
            event = self._event_cache[ref_uuid]
            if event.desc.poll_desc.stopped:
                message = "(event - %s) - event already stopped" % (
                    event.identify())
                LOG.info(message)
            else:
                self._controller.poll_add(
    ...
    def schedule_graph_events_by_ids(self, event_ids):
        for event_id in event_ids:
            try:
                ...
                kerr = kerr
                message = "%s - Graph event missing in cache" % (
                    event_id)
                LOG.error(message)
    ...
    def process_events(self, events, dispatch=True):
