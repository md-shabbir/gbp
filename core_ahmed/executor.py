...
from gbpservice.nfp.core_ahmed import threadpool as core_tp
...
class EventGraphExecutor(object):
    ...
    def execute(self, graph):
        ...
        to_run = self._leaf_nodes(graph['data'], graph['root'])
        self.manager.schedule_graph_events_by_ids(to_run)


    def dispatch(self, node):
        # Check if this node can be dispatched...
        # If there are child nodes to be processed,
        # this node cannot be dispatched
        graph = self._graph(node)
        if not graph:
            return True

        to_run = self._leaf_nodes(graph['data'], node)
        if to_run == [node]:
            return True
    ...
    def _leaf_nodes(self, tree, root):
        # Get the lowest level leafs from the root
        run = []
        try:
            nodes = tree[root]
            if nodes == []:
    ...
    def conntinue(self, completed_node, graph=None):
        graph = self._graph(completed_node)
        if graph:
            #graph = self.running[graph['id']]
            if completed_node == graph['root']:
                ...
                if graph['data'][root] == []:
                    self.manager.schedule_graph_events_by_ids([root])
