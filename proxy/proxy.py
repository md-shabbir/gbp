...
def proxy(unix_socket):
    ...
    tcp_server_address = ('172.16.0.3', 8070)
    try:
    ...
    print "@@@ PROXY - unix - %d, tcp - %d" % (
        unix_socket.fileno(), tcp_socket.fileno())
    ...
    try:
        ...
        for s in writable:
            ...
            try:
                ...
                print >>sys.stderr, 'Queue for socket - %d, data len %d' % (
                    proxy_map[s].fileno(), len(next_msg))
            except Queue.Empty:
                # No messages waiting so stop checking for writability.
                # print>>sys.stderr,'outputqueue for',s.getpeername(),'isempty'
                print >>sys.stderr, "socket - %d is writable" % (s.fileno())
                print >>sys.stderr, 'Queue for socket - %d is empty' % (
                    proxy_map[s].fileno())
            except Exception as exc:
                import pdb;pdb.set_trace()
            else:
                try:
                    ...
                    print "@@@ Socket - %d, got exception in sending - %s" % (
                        s.fileno(), exc)
        ...
        for s in readable:
            ...
            if s is server:
                ...
                if data:
                    msg = 'Queue for socket - %d, updating len - %d' % (
                        s.fileno(), len(data))
                ...
                # print 'RECEIVED: ', data
                    ...
                    msg = 'closing %s after reading no data' % client_address
                    print >>sys.stderr, msg
            ...
            for s in exceptional:
                msg = 'handling exceptional condition for %s' % s.getpeername()
                print >>sys.stderr, msg
